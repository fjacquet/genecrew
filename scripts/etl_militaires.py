...
def classify_and_rows(path: Path):
    ...
    for member, stream in open_csvs(path):
        ...
        if any(f.startswith("bloc_etat_civil") for f in fields):
            ...
            print(
                f"  ? format inconnu: {path.name}::{member} ({fields[:4]})",
                file=sys.stderr,
            )
            continue
        for row in reader:
            if fmt == "A":
                yield {
                    "base": pick(
                        row, "id_conflit_intitule", "id_sous_conflit_intitule"
                    ),
                    "nom": pick(row, "nom"),
                    ...
                    "naissance_lieu": pick(
                        row,
                        "id_naissance_lieu_intitule",
                        "naissance_autre_circonscription",
                    ),
                    "naissance_departement": pick(
                        row, "id_naissance_departement_intitule"
                    ),
                    "naissance_pays": pick(row, "id_naissance_pays_intitule"),
                    "deces_date": iso_date(
                        pick(
                            row, "deces_jour_mois_annee", "disparition_jour_mois_annee"
                        )
                    ),
                    "deces_lieu": pick(
                        row,
                        "id_deces_lieu_intitule",
                        "deces_lieu_intitule",
                        "deces_autre_lieu",
                    ),
                    "deces_pays": pick(row, "id_deces_pays_intitule"),
                    "unite": pick(
                        row, "id_unites_intitule", "unite", "id_grade_intitule"
                    ),
                    "reference": " / ".join(
                        filter(
                            None,
                ...
                yield {
                    "base": pick(
                        row,
                        "bloc_etat_civil.liaison_liste_conflit:titre",
                    ...
                    "deces_lieu": pick(
                        row, "bloc_deces.liaison_liste_lieu_deces:titre"
                    ),
                    "deces_pays": pick(
                        row, "bloc_deces.liaison_liste_pays_deces:titre"
                    ),
                    "unite": pick(
                        row,
                        "bloc_etat_civil.unite",
                    ...
                    "lien_ark": pick(
                        row,
                        "bloc_etat_civil.lien_ark_image",
...
def main() -> None:
    ...
    lines = [
        "# Inventaire données militaires (Mémoire des hommes)",
        "",
    ...
    for name, c in per_file.items():
        ...
        lines.append(
            f"| {name} | {c['lignes']} | {c['naissance_complete'] * 100 // n}% "
