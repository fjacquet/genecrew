...
def test_evenement_cree_et_rattache(monkeypatch):
    _stub_outil(
        monkeypatch,
        {"success": True, "data": {"handle": "EV1", "created": True, "attached": True}},
    )
...
def test_simulation_n_est_pas_un_orphelin(monkeypatch):
    ...
    res = creer_evenement_source(
        "H1", event_type="Death", dateval=[23, 12, 2021], dry_run=True
    )
    assert res["posee"] is True
...
def test_handle_synthetique_seul_suffit_a_reconnaitre_la_simulation(monkeypatch):
    ...
    _stub_outil(
        ...
        {
            "success": True,
            "data": {"handle": "DRYRUN:event", "created": False, "attached": False},
        },
    )
