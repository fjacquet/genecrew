...
from genecrew.lieux_wiki import (
    has_wikipedia_url, pick_article, run_lieux_wiki, title_core,
)
...
def test_pick_article_verifies_name_and_prefers_distance():
    cands = [{"title": "Hanancha", "dist": 5066.3},
             {"title": "Tiffech", "dist": 3.8}]
...
def test_pick_article_abstains_on_two_distinct_matches():
    cands = [{"title": "Sidi Fredj", "dist": 100.0},
...
PLACE = {"handle": "HT", "gramps_id": "P0358", "name": {"value": "Tiffech"},
         "lat": "36.1917", "long": "7.7861", "urls": [], "media_list": [],
         "_class": "Place", "place_type": "Municipality"}
...
def test_run_lieux_wiki_links_and_images_verified(tmp_path, monkeypatch, mocker):
    ...
    monkeypatch.setattr(lieux_wiki, "frwiki_geosearch",
                        lambda lat, lon, **kw: [{"title": "Tiffech", "dist": 3.8}])
    monkeypatch.setattr(lieux_wiki, "frwiki_page_info", lambda t, **kw: {
        "title": "Tiffech", "url": "https://fr.wikipedia.org/wiki/Tiffech",
    ...
    assert any(u.get("path") == "https://fr.wikipedia.org/wiki/Tiffech"
               for b in place_puts for u in b.get("urls", []))
    assert any(any(r.get("ref") == "MED1" for r in b.get("media_list", []))
               for b in place_puts)
    media_put = [b for p_, b in state["puts"] if p_ == "/api/media/MED1"]
    assert "Wikimedia Commons" in media_put[0]["desc"]         # attribution


def test_limit_bounds_the_gramps_fetch_not_only_the_filter(tmp_path, monkeypatch, mocker):
