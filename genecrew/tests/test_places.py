...
def test_run_places_readonly_writes_reports_no_http_write(
...
def test_run_places_continues_past_resolver_http_error(tmp_path, monkeypatch):
    ...
    )  # le run se termine, le lieu est indécidable
...
def test_run_places_indecidable_when_unresolved(tmp_path, monkeypatch):
    ...
    assert (
        props
        and props[0]["type"] == "lieu_indecidable"
        and props[0]["resolution"] is None
    )
    ...
    md = render_places_report(
        ...
        [
            PlaceProposition(
                ...
                original="…",
                country="France",
                resolution=_authoritative(None),
                action="ecrire",
                confiance="haute",
                priorite="haute",
                preuve="geo.api.gouv.fr | /communes/18033 | score 1.000",
            )
        ],
    )
