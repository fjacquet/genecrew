...
SYLVAIN_ROW = {
    ...
    "nom": "VILLAUDY",
    "prenom": "Sylvain",
    "naissance_date": "1895-11-11",
    "naissance_lieu": "Paris 20e",
    "naissance_departement": "Seine",
    "naissance_pays": "France",
    "deces_date": "1915-09-28",
    "deces_lieu": "Neuville-Saint-Vaast",
    "deces_pays": "France",
    "unite": "156e RI",
    "reference": "cl.1915",
...
def test_run_militaires_full_birth_match_proposes_with_ark(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        militaires, "iter_people_batches", lambda *a, **k: iter([[sylvain]])
    )
    monkeypatch.setattr(
        militaires, "match_militaires", lambda *a, **k: [(SYLVAIN_ROW, 1.0)]
    )
    report, proposals = run_militaires(
        client=None, scope="all", output_dir=tmp_path, date="2026-07-19"
    )
...
def test_run_militaires_year_only_and_ambiguous_are_dropped(tmp_path, monkeypatch):
    ...
    def fake_match(surname, given, birth_iso, **kw):
        if surname == "Jacquet":
            return [
                ({**SYLVAIN_ROW, "nom": "JACQUET"}, 0.85)
            ]  # année seule: sous seuil
        return [
            ({**SYLVAIN_ROW, "nom": "CLAVIER"}, 0.96),
            ({**SYLVAIN_ROW, "nom": "CLAVIER", "prenom": "Louis A."}, 0.93),
        ]

    monkeypatch.setattr(militaires, "match_militaires", fake_match)
    _, proposals = run_militaires(
        client=None, scope="all", output_dir=tmp_path, date="2026-07-19"
    )
...
def test_run_militaires_exact_death_rescues_source_mode(tmp_path, monkeypatch):
    p = _person(
        ...
        death=_event("Death", 1915, 28, 9),
    )  # décès exact concordant
    monkeypatch.setattr(militaires, "iter_people_batches", lambda *a, **k: iter([[p]]))
    monkeypatch.setattr(
        militaires, "match_militaires", lambda *a, **k: [(SYLVAIN_ROW, 0.85)]
    )
    _, proposals = run_militaires(
        client=None, scope="all", output_dir=tmp_path, date="2026-07-19"
    )
