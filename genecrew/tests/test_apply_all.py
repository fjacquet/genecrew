...
PLACES = [
    {"handle": "hp1", "gramps_id": "P0001", "name": {"value": "Bourges, Cher, France"},
...
def _authoritative_place(place, min_score):
    ...
    rp = ResolvedPlace(name="Bourges", place_type="Municipality", lat="47.081", long="2.399",
                       code="18033", chains=[], score=1.0, source="test", query="test")
    return PlaceProposition(type="lieu_resolu", gramps_id=place["gramps_id"], handle=place["handle"],
                            ...
                            action="ecrire", confiance="haute", priorite="haute", preuve="test")
...
def test_run_apply_all_runs_all_three_steps(tmp_path, mocker):
    ...
    def on_write(request):
        ...
        if request.method == "POST":
            return httpx.Response(201, json={"handle": "H_" + payload.get("name", {}).get("value", "new")})
