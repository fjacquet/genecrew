...
def test_candidate_selection():
    ...
    unsourced = _person(
        "I2", "h2", "A", "B", birth=_event("Birth", 1950), death=_event("Death", 2000)
    )
    sourced = _person(
        "I3",
        "h3",
...
def test_year_only_match_never_proposes_a_death(tmp_path, monkeypatch):
    ...
    paul = _person(
        "I0161", "h161", "Paul, Marcel", "Jacquet", birth=_event("Birth", 1945)
    )  # année seule
...
def test_divergent_death_with_year_only_birth_is_dropped_not_contradiction(
    ...
    roger = _person(
        ...
        "Roger",
        "Jacquet",
        birth=_event("Birth", 1920),
        death=_event("Death", 1961, 29, 9),
    )
    insee = {
        "name": {"first": ["Roger"], "last": "Jacquet"},
        "birth": {"date": "19200504"},
        "death": {"date": "19870908"},
    }
...
def test_run_deces_queries_candidates_scores_and_writes_yaml(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        deces, "iter_people_batches", lambda *a, **k: iter([[odette, ancien]])
    )
    calls = []
