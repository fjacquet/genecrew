...
def render_gender_report(
    scope, date, propositions, indecidables, people_count, base_url="http://localhost"
) -> str:
    ...
    lines = [
        f"# Inférence de genre — {scope} — {date}",
        "",
        "## Synthèse",
        "",
    ...
    if props:
        lines += [
            "| Personne | Type | Actuel | Proposé | Confiance | Priorité | Preuve |",
            "|---|---|---|---|---|---|---|",
        ]
        for p in props:
            lines.append(
...
def render_propositions_yaml(propositions: list[Proposition]) -> str:
    ...
    return yaml.safe_dump(
        [p.model_dump() for p in propositions], allow_unicode=True, sort_keys=False
    )


def _build_proposition(person, inf) -> Proposition:
    preuve = (
        f"prénom « {inf.key} » : {inf.ratio * 100:.1f}% "
    ...
    if person.sex == "U":
        return Proposition(
            ...
            valeur_actuelle="U",
            valeur_proposee=inf.sex,
            preuve=preuve,
            confiance=_confiance(inf.ratio),
            priorite="moyenne",
        )
    return Proposition(
