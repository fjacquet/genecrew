...
def render_apply_report(
    scope, date, applied, below, errors, dry_run, base_url="http://localhost"
) -> str:
    ...
    lines = [
        f"# Application des corrections de genre — {scope} — {date}",
        "",
        f"Mode : {mode}.",
        "",
    ...
    if applied:
        lines += [
            "| Personne | Nom | Type | Ancien | Nouveau | Ratio | Preuve |",
            "|---|---|---|---|---|---|---|",
        ]
        for gid, personne, typ, old_i, new_i, ratio, preuve in applied:
            lines.append(
                f"| {_link(gid, base_url)} | {personne} | {typ} | {_INT_TO_SEX[old_i]} "
    ...
    if below:
        lines += [
            "| Personne | Nom | Prénom | Sexe inféré | Ratio |",
            "|---|---|---|---|---|",
        ]
        for gid, personne, given, sex, ratio in below:
            lines.append(
                f"| {_link(gid, base_url)} | {personne} | {given} | {sex} | {ratio:.3f} |"
            )
    else:
        lines.append("Aucun.")
    lines += ["", "## Erreurs", ""]
    ...
    *,
    ...
    for batch in iter_people_batches(client, fetcher, scope, batch_size, limit):
        for p in batch:
            ...
            payload = json.loads(
                tool._run(handle=p.handle, gender=_SEX_TO_INT[inf.sex], dry_run=dry_run)
            )
            if payload["success"]:
                d = payload["data"]
                applied.append(
                    (p.gramps_id, p.name, typ, d["old"], d["new"], inf.ratio, preuve)
                )
            else:
