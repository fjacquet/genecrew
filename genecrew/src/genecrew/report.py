...
def render_report(
    ...
    if anomalies:
        ordered = sorted(
            anomalies, key=lambda a: (_SEVERITY_ORDER.get(a.severity, 9), a.gramps_id)
        )
        lines += ["| Personne | Sévérité | Règle | Détail |", "|---|---|---|---|"]
        for a in ordered:
            lines.append(
                f"| {_link(a.gramps_id, base_url)} | {a.severity} | {a.rule} | {a.message} |"
            )
    else:
    ...
    if duplicates:
        ...
        for d in sorted(duplicates, key=lambda x: -x.score):
            lines.append(
                f"| {_link(d.gramps_id_a, base_url)} | {_link(d.gramps_id_b, base_url)} "
                f"| {d.score} | {d.reason} |"
            )
    else:
